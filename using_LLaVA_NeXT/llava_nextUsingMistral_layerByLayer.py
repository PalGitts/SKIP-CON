...
#     print(f'embeddings_pca: {embeddings_pca.shape}')
#     1/0
...
def iterate_overPrompts(prompt_list, trg_mode, trg_layer):
    ...
    for idx, prompt in enumerate(prompt_list):
        ...
        temp = torch.unsqueeze(allHS, 0).to('cpu')
        if pile == None:
            pile = temp
        else:
            pile = torch.cat([pile, temp], dim=0)

        if idx % 3 ==0 and idx > 0:
            print(f'pile: {pile.shape}')
            fragment = pca.fit_transform(pile.to(dtype=torch.float32).cpu().numpy())
            pile = None
            

            if embeddings_pca is None:
                embeddings_pca = fragment
            else:
                # embeddings_pca = torch.cat([embeddings_pca, fragment], dim=0)
                embeddings_pca = np.concatenate((embeddings_pca, fragment), axis=0)
...
for layer_idx in range(0,32):
    ...
    upto = 101
    safeTensors = iterate_overPrompts(safeList[:upto], 'BENIGN', layer_idx)
    ben_colors = ['blue'] * safeTensors.shape[0]
    ...
    for mal_trg_mode in ['BLACK', 'DISABLE', 'ISLAM', 'LGBTQ', 'WOMEN', 'TOXIC']:
        ...
        clear_output(wait=True)
        unsafeTensors = iterate_overPrompts(unsafeList[:upto], mal_trg_mode, layer_idx)
        mal_colors = ['red'] * unsafeTensors.shape[0]
        colors = ben_colors + mal_colors

        # embeddings_pca = torch.cat( [safeTensors, unsafeTensors], dim=0 )
        emb_pca = np.concatenate((safeTensors, unsafeTensors), axis=0)
        plot_embeddings(emb_pca, layer_idx, colors, mal_trg_mode=mal_trg_mode, ben_trg_mode='BENIGN')
